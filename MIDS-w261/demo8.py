...
# MAGIC %md
# MAGIC # wk8 Demo - Advanced Spark - Pipelines and Optimizations with DataFrames
...
#%scala
# Read the data
textFile = spark.read.textFile("/databricks-datasets/samples/docs/README.md")
textFile.count()

#Transform the Data and run a foreach
...
# MAGIC %md
# MAGIC # Exercise 1. Structured API - Datasets, DataFrames, and SQL Tables and Views
# MAGIC 
...
# MAGIC * Why would we want to use RDDs in this class over DataFrames?
# MAGIC > custom partitioning; some things can only be done in the RDD API
# MAGIC * What is a UDF? Why do we need to create them?
# MAGIC > User Defined Function; to extend the Spark functionality; to perform row-by-row operations
# MAGIC * What is vectorized computation and how does that differ from row-by-row function calls
# MAGIC > computations are executed concurrently. This is very efficient. For example, use numpy when you can. numpy is implemented in C, and C implements vectorized operations
# MAGIC * How is a Dataset different than a DataFrame?
# MAGIC > Datasets are strongly typed. Datasets have a slightly richer API. Can be more optimized due to type casting.
# MAGIC * Are Datasets avaliable in the Python API?
# MAGIC > No. Python is a dynamically typed language and as such is not comaptible with datasets

# COMMAND ----------

# MAGIC %md
# MAGIC # Exercise 2. Data Serialization Formats. 
...
# MAGIC %md
# MAGIC ## First let's understand our data
# MAGIC 
# MAGIC Here we see that we have several compressed CSV files as we expect based on our bq command specifying compressions. BigQuery was nice enough to split the files into 30 MB chunks so that our analysis will be partitioned nicely for ingestion.
# MAGIC 
...
# MAGIC %md
# MAGIC Wow that's nice we didn't even have to handle the decompression and it saves a ton on disk space! Next we're going to save this in a few different serializations so that we can see the effect on disk space.
# MAGIC 
...
# MAGIC %md
# MAGIC ### How do these look?

# COMMAND ----------

# MAGIC %md
# MAGIC We have 4 data types below
# MAGIC 
...
# MAGIC %md
# MAGIC ### How do these compare for simple computations?

# COMMAND ----------

# MAGIC %md
# MAGIC First we need to read in the data again to ensure we're working with non-cached versions
...
# MAGIC %md
# MAGIC ### Count: 
# MAGIC Parquet keeps metadata about the data in order to compute some calculations extremely quickly such as row counts. Notice the difference in wall time vs CPU times. Reading in parquet (I/O) is much more efficient!
...
# MAGIC %md
# MAGIC ### Average of a column: 
# MAGIC Parquet is column oriented so it can go through the sequence of data in one step instead of taking each row. This should have much higher performance. On the other hand, there is some overhead in writing the data to parquet in the first place. As always, there are tradeoffs!
...
# MAGIC %md
# MAGIC > __DISCUSSION QUESTIONS:__ For each key term from the reading, briefly explain what it means in the context of this demo code. Specifically:
...
# MAGIC  * _Which types (data types) of columns do you think has the best compression for parquet?_
...
# MAGIC  * _If you had to partition data into days as new data comes in with aggregations happening at end of day how would you operationalize this? For example, we collect weather data throughout the day, and at the end of the day we'd like to do some analysis_
...
# MAGIC %md
# MAGIC # Exercise 3. Working with DataFrames and simple User-Defined Functions (UDFs)
# MAGIC 
...
# MAGIC %md
# MAGIC There's many things we could do from here but there are some important performance considerations when using UDFs. 
# MAGIC 
# MAGIC 
...
# MAGIC 
# MAGIC * What questions would you ask of this table?
# MAGIC > What is the change in temperature over time as a function of lat/long. In other words, Do we have global warming? Let's look at it by looking at average tempertues by latitude have evolved over time.
# MAGIC 
...
display(stations_us.where(F.col('usaf')==722280))

# COMMAND ----------

# MAGIC %md
# MAGIC There are two methods to perform this operation: you can use `where` or `filter` (as above) and they will both perform the same operation. (pg 74 - Spark, The Definitive Guide). Remember that the DataFrame API and Spark SQL compile to the same execution plan.
# MAGIC 
# MAGIC Take a look at the explain plan for pushdown predicates: (`PushedFilters: [IsNotNull(station_number)]`). (More info on pg 169, 325 - Spark, The Definitive Guide). Spark pushes down predicates to the data layer.
...
# MAGIC %md
# MAGIC Spark will automatically broadcast a small table, it's usually best to let Spark decide. See explain plan below. Notice that Spark broadcast the join for us even though we took that function out in our code. (p.151 Defintive Guide)
